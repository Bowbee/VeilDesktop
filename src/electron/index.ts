...
const handleInstallPack = async (args: { name: string; url: string; version: string }) => {
  // console.log('installing pack', args);
  const dir: string = settings.get("wowDirectory");
  if (!dir) {
    await handleSetWoWDirectory();
  }
  const packFolder = path.join(dir, "_retail_", "Interface");
  const veilStagingDir = path.join(dir, "_retail_", "Interface", "VeilApp", "Staging");
  // console.log(veilStagingDir);
  if (!fs.existsSync(veilStagingDir)) {
    fs.mkdirSync(veilStagingDir, { recursive: true });
  ...
  files.forEach((folderName) => {
    ...
    const resourceFolderPath = path.join(packFolder, folderName);
    // is this a folder?
    if (!fs.lstatSync(path.join(veilStagingDir, folderName)).isDirectory()) {
      return;
    }
    const backupPath = path.join(veilBackup, folderName);
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const promise = new Promise((resolve, _reject) => {
      ...
      if (fs.existsSync(resourceFolderPath)) {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        sudo.exec(`robocopy "${resourceFolderPath}" "${backupPath}" /E /MOVE`, options, (_error, stdout, _stderr) => {
          resolve(stdout);
        });
      } else {
        resolve(true);
  ...
  // copy staging to addon folder
  files.forEach((folderName) => {
    // const addonFolderPath = path.join(packFolder, folderName);
    // if (fs.existsSync(addonFolderPath)) {
    //   // console.log("DIDNT BACK UP??");
    //   throw new Error("Didnt back up Addon. Something went wrong.");
    // } else {
    // }
    fs.renameSync(path.join(veilStagingDir, folderName), path.join(packFolder, folderName));

  });
  // delete staging folder
  // fs.rmSync(veilStagingDir, { recursive: true });
  let installed: { [name: string]: string } = settings.get("installedPacks");
  if (!installed) {
    installed = {};
  }
  installed[args.name] = args.version;
  settings.set("installedPacks", installed);
  // console.log(installed);
  return installed;
};
  ...
  if (canceled) {
    ...
    if (files.includes("Data") && files.includes("_retail_")) {
      ...
      new Notification({
        ...
        body: "The directory you selected is not a valid World of Warcraft installation.",
      }).show();
...
app.on("ready", () => {
  ...
  ipcMain.handle("pack:install", (_event, args: { name: string; url: string; version: string }) => {
    return handleInstallPack(args);
  });
  ipcMain.handle("pack:installed", () => {
    const installed: { [name: string]: string } = settings.get("installedPacks");
    return installed || {};
  });
  createWindow();
});
